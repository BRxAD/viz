...
Do NOT generate any fake text or hallucinated letters. Use clean visuals, icons, and shapes only. Avoid fake paragraphs or distorted writing. The layout should be visually clear, infographic-style, with labeled sections.`;
    ...
    // Step 1: Ask GPT to suggest tool call
          ...
          function: {
            ...
            parameters: {
              ...
              properties: {
                ...
                response_format: { type: "string", enum: ["url"] },
              },
    ...
    // Step 2: Manually execute the tool call (fake tool execution)
    const toolCalls = chatResponse.choices[0].message.tool_calls;

    if (!toolCalls || toolCalls.length === 0) {
      throw new Error('No tool call returned by the model.');
    }

    const toolArguments = JSON.parse(toolCalls[0].function.arguments);

    // Step 3: Actually generate image manually
    const dalleResponse = await openai.images.generate({
      model: "dall-e-3",
      prompt: toolArguments.prompt,
      size: toolArguments.size,
      style: toolArguments.style,
      response_format: toolArguments.response_format,
    });
